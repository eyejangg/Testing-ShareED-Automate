...
ภาพรวมและสถาปัตยกรรมการทดสอบ (Architecture & Lifecycle)
สรุปสิ่งที่เราได้พัฒนาและปรับปรุงทั้งหมด (What We Have Done)
เจาะลึก 8 Test Cases (8 Test Cases Deep Dive)
เจาะลึกโครงสร้างโค้ดและการทำงานแต่ละไฟล์ (Code Deep Dive)
...
4.5 tests/posts/01-create-post.spec.js (ชุดทดสอบหลัก 8 เคส)
โครงสร้างไดเรกทอรีและไฟล์สำคัญ (Project Structure)
คู่มือคำสั่งใช้งานและส่งออกผลการทดสอบ (CLI & Export Guide)
เทคนิคและ Best Practices ที่นำมาใช้
...
Accessibility-based Locators (getByRole, getByLabel): ค้นหาปุ่มและช่องกรอกตามมาตรฐาน เข้าถึงง่ายและไม่พังเมื่อโครงสร้าง HTML เปลี่ยน
Scrolling Resilience (scrollIntoViewIfNeeded): สั่งเลื่อนหน้าจอลงไปมองปุ่มหรือข้อความทุกครั้งก่อนทำการ Assert เพื่อแก้ปัญหาหน้าจอเรนเดอร์ไม่ทัน
Idempotency & Zero Flaky Tests: การมี Global Setup และ Teardown ช่วยให้รันกี่รอบก็ได้ผลลัพธ์ผ่าน 100% เหมือนเดิมเสมอ ไม่มีข้อมูลตกค้าง
Comprehensive Data Flow: ผูกข้อมูลต่อเนื่อง (Create ➔ Edit ➔ Delete) ทำให้ทดสอบระบบได้เสมือนผู้ใช้งานจริง 100%
